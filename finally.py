...
#joblib.dump(model, "ganzhiji_model.m") #保存模型
...
model = joblib.load("ganzhiji_model.m") #调用模型
...
data_cancha = pd.concat([a2,a1],axis=1)
...
#读震动信号
file_path = "C:/Users/1701/Desktop/前轴内损伤/84"
...
for  i in b:
    s = i
    data_time = datetime.datetime(year=int(s[17:21]), month=int(s[21:23]),  day=int(s[23:25]),hour=int(s[26:28]),minute=int(s[28:30]),second=int(s[30:32]))
    q.append(data_time.strftime("%Y-%m-%d %H:%M:%S"))
#震动信号的时间chuo
w = []  
import time 
for i in q:
...
for i in zd:
    b1 = list(re.sub("\D", "", timeStamp(i)) )
    b1.insert(8,"-")#只能跑一次
    b1  = 'C:/Users/1701/Desktop/前轴内损伤/84\\1076701-84-501-6_' + ''.join(b1) + '.txt'  #84和21需要改
    y.append(b1)
    
#取出震动信号数据
csv_list = glob.glob('C:/Users/1701/Desktop/前轴内损伤/84/*.txt') 
...
y2 =  y1.reset_index(drop=True) #去掉原索引，重新生成索引
ff_data = pd.concat([y2,f_data],axis=1)

ff_data.to_csv('C:/Users/1701/Desktop/shi_ping.csv',index=False)
...
##    im4=col
###中值滤波
##    data_F=signal.medfilt(im4,3)# 只能以奇数进行赋值
##    f.append(data_F)
####进行σ = 2的高斯滤波
...
##a=pd.DataFrame(f).T
##plt.scatter(a.iloc[:,3],a.iloc[:,21],s=4)
